 ...
 * Modified 2026-08-12 by 江导项目: safer video linking, local branding,
 * and project-local server instructions.
 *
 * 生成审核网页（视频版本）
 *
...
if (videoFile !== videoBaseName && fs.existsSync(videoFile)) {
  ...
  if (fs.existsSync(videoBaseName)) {
    const existing = fs.lstatSync(videoBaseName);
    if (!existing.isSymbolicLink()) {
      console.error('❌ 当前目录已有真实 video.mp4；请换到独立审核目录，避免覆盖素材。');
      process.exit(1);
    }
    fs.unlinkSync(videoBaseName);
  }
...
<body>
  ...
  <div class="header">
    <div class="header-left">
      <span class="logo"><span class="icon">&#9986;</span> 江导剪辑 · ${path.basename(videoFile)}</span>
...
console.log('📌 启动服务器: node <jiangdao-edit>/scripts/review-server.js 8899 "$VIDEO_PATH"');
