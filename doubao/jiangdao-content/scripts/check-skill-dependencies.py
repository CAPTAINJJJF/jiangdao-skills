...
sys.path.insert(0, str(PROJECT_SKILLS / 'shared' / 'scripts'))
from runtime_config import skill_root
GLOBAL_SKILLS = skill_root()
...
def main() -> int:
    ...
    print(f"DEPENDENCY_OK {len(DEPENDENCIES)}/{len(DEPENDENCIES)} RESOURCES_OK {len(resources)} DISCOVERY={'not_checked' if args.source_only else 'paths_verified'}", flush=True)
