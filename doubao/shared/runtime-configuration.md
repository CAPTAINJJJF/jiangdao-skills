...
两版使用同源规则和脚本，豆包包仅移除 `agents/openai.yaml`。运行脚本优先使用显式 `--global-skills`，其次使用 `JIANGDAO_SKILLS_DIR` 或 `DOUBAO_SKILLS_DIR`，未设置时从当前脚本推导同一安装包的根目录；开发者通过符号链接将同一份脚本注册到 Codex 时保留该安装入口，以便找到同级扩展，不借用内容相同但位置不同的其他安装。路径验证只证明文件可读取；首次安装后仍需在对应应用的新任务中确认技能实际发现与调用。

`shared/` 在两版分发目录中各有一份内容相同的副本，使豆包清单和相对引用在自身目录内成立，不通过跨目录链接或符号链接借用另一版。
