...
import subprocess
import sys
from runtime_config import configured_path, downloader_path
...
def main():
    ...
    p.add_argument('--global-skills', type=Path, default=Path(os.environ.get('CODEX_HOME', str(Path.home()/'.codex'))) / 'skills')
    args = p.parse_args()
    missing = []; notes = []
    def command(name):
